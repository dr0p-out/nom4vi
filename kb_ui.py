...
class InputArea(QPlainTextEdit):
  ...
  def keyPressEvent(self, ev: QKeyEvent):
    ...
    if modifiers == Qt.KeyboardModifier.NoModifier:
      ...
      if Qt.Key.Key_A <= ev.key() <= Qt.Key.Key_Z:
        self.onDoLetter.emit(ev.text())
        return
...
class EditorWindow(QWidget):
  ...
  def __init__(self):
    ...
    fontBtn = QPushButton()
    barLayout.addWidget(fontBtn)
    fontBtn.setText(self.tr('Font…'))
    fontBtn.setEnabled(False)
    fontBtn.clicked.connect(self.__handleSetFont)
    ...
    self.__lbl = lbl
    lbl.setToolTip(self.tr('The current incomplete word you\'ve inputted.'))
  ...
  def setEngine(self, engine: BaseEngine):
    if engine is None:
